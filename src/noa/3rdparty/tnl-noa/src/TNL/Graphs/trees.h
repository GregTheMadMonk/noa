...
#include <noa/3rdparty/tnl-noa/src/TNL/Algorithms/find.h>
      ...
      if( std::is_same_v< DeviceType, Devices::Sequential > ) {
         ...
         while( visited_old != visited ) {
            ...
            if constexpr( MatrixType::isSymmetric() )
               graph.getAdjacencyMatrix().reduceAllRows( symmetric_fetch, noa::TNL::Plus{}, keep, (IndexType) 0 );
            else
               graph.getAdjacencyMatrix().reduceAllRows( fetch, noa::TNL::Plus{}, keep, (IndexType) 0 );
